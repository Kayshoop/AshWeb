document.addEventListener('DOMContentLoaded', function() {
    ...
    const serviceList = document.getElementsByClassName('serviceList')[0];
    ...
    services.forEach(service => {
        ...
        serviceBox.addEventListener('click', function() {
            displayService(service.title, service.price);
        });

        gallery.appendChild(serviceBox);
    });

    function displayService(title, price) {
        ...
        selectedService.innerHTML = `
            ...
            <p>${price}</p>
        `;
        serviceList.appendChild(selectedService);
    }
});
